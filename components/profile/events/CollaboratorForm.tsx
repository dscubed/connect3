...
export default function CollaboratorForm({ collaborators, setCollaborators, disabled = false }: CollaboratorFormProps) {
  const DEBOUNCE_TIMEOUT = 300;
  const { user } = useAuthStore();
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [debouncedQuery, setDebouncedQuery] = useState<string>("");
  const [searchResults, setSearchResults] = useState<Array<{id: string, name: string, avatar_url?: string}>>([]);
      ...
      try {
        ...
        if (error) {
          ...
          const sorted = data?.sort((a, b) => {
            const aStarts = a.first_name.toLowerCase().startsWith(debouncedQuery.toLowerCase());
            const bStarts = b.first_name.toLowerCase().startsWith(debouncedQuery.toLowerCase());
         ...
         {searchResults.length > 0 && (
           <div className="border rounded p-2 max-h-40 overflow-y-auto">
             {searchResults.map((result) => (
               <div key={result.id} className="flex items-center justify-between p-2 hover:bg-white/5 select-none">
                 <span>{result.name}</span>
                 <Button
                   size="sm"
                   onClick={() => addCollaborator(result)}
                   disabled={disabled || collaborators.some(c => c.id === result.id)}
                 >
                   Add
                 </Button>
               </div>
