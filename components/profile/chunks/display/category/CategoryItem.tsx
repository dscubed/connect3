    ...
    <SortableCategory key={category} id={category}>
      {({ attributes, listeners, setNodeRef, style }) => (
        ...
        >
          ...
          <div
            className="flex items-center mb-2 gap-2 hover:cursor-grab hover:bg-white/10 py-1 rounded-md w-full"
            {...attributes}
            {...listeners}
          >
            <h1 className="text-lg font-semibold flex items-center justify-center">
              {isEditing && (
                <GripVertical className="h-5 w-5 mr-1 text-white/50 onhover:cursor-grab" />
              )}
              {category}
            </h1>
            {isEditing && (
